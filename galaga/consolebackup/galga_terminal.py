...
class Game:
    ...
    def maybe_start_dive(self, living):
        ...
        if diver.glyph == "M" and random.random() < 0.75:
            ...
            self.beam_frames = random.randint(16, 30)
            self.beam_length = 1
    ...
    def maybe_capture_player(self):
        ...
        if contact_depth < 0 or contact_depth >= self.beam_length:
            return
    ...
    def move_enemies(self):
        ...
        for e in living:
            fire_chance = ENEMY_FIRE_CHANCE + (self.level * 0.002)
            if e.diving:
                fire_chance *= 2.2
    ...
    def collisions(self):
        ...
        for b in self.bullets:
            if not b.from_enemy:
                ...
                if hit:
                    if self.capture_revenge_target is hit:
                        self.dual_shot = True
                        self.capture_revenge_target = None
                        self.captured_ship_active = False
                        self.captured_ship_enemy = None
